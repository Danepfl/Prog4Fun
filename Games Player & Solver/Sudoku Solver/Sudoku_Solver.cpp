...
vector <vector <vector <int> > > option3(vector <vector <vector <int> > > CoordM, int *optback3, int z, int i){//choisit les options entre 3 options
    //retourne 1 si pas de sol i.e. sudoku insoluble
    //retourne 2 si plusieurs solutions
    //retourne 0 si normal
...
DUPERLOOP:
        ...
        for(int l=0; l<3; l++){
            for(int m=0; m<3; m++){
                for(int j=0; j<3; j++){
                    ...
                    string tmp_sol = to_string(CoordM[0][a][6]);
                    cout << CoordM[0][a][6]<< " ";
    ...
    cout << endl << endl;
    cout << solution; // For the Labview part
}//fin du main
