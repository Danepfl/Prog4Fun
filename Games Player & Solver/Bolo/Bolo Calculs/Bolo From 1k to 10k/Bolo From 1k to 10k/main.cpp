...
int main() {//Resultats finaux: f1=0.05; f2=3; f3=1;
    ...
    vector <double> fvec4;
    vector <double> fvec5;
        /*for(beta1=0;beta1<=0.000001;beta1=beta1+0.00000001){//101 comb
            fvec1.push_back(beta1);//Avant 0.00000004
            fvec2.push_back(0);
        }*/
        for(gamma1=0;gamma1<=2;gamma1=gamma1+0.02){//101 comb
                fvec1.push_back(0);
                fvec2.push_back(gamma1);
        }
    fvec1.resize(64);
    fvec2.resize(64);
    fvec1.shrink_to_fit();
    fvec2.shrink_to_fit();
    for(j=0;j<fvec1.size();j++){
        //cout << fvec1[j] << " " << fvec2[j] << endl;
    }
...
RESTART:
    while(fvec1.size()!=0){
        ...
        ford1[0]=fvec1[no1]; ford1[1]=fvec2[no1];
        ford2[0]=fvec1[no2]; ford2[1]=fvec2[no2];
        ford3[0]=fvec1[no3]; ford3[1]=fvec2[no3];
        ford4[0]=fvec1[no4]; ford4[1]=fvec2[no4];
        ...
        for(partie=0;partie<counttour;partie++){
            scoreordi=0; scordi1=0; scordi2=0; scordi3=0; scordi4=0;
                    ...
                    for(t=1;t<=4;t++) {
                        ...
                        while(z==0){            //si z=0, le tour continue, si l'ordi s'arrete/perd => z=1 et le tour de l'ordi se termine
                            ...
                            switch (tourordi) {
                                case 1:
                                    scoreordi=scordi1;
                                    beta1=ford1[0];gamma1=ford1[1];
                                    break;
                                case 2:
                                    scoreordi=scordi2;
                                    beta1=ford2[0];gamma1=ford2[1];
                                    break;
                                case 3:
                                    scoreordi=scordi3;
                                    beta1=ford3[0];gamma1=ford3[1];
                                    break;
                                case 4:
                                    scoreordi=scordi4;
                                    beta1=ford4[0];gamma1=ford4[1];
                                    break;
                                default:
                                    break;
                            }
                            ...
                            if(perdu!=1){//Si l'ordi n'a pas perdu à ce tour
                                ...
                                if ((scoreordi<1000)&&(scoreordi>=0)&&(scoremeilord<=10000)){   //cas au debut de la partie, avant d'avoir 10'000 pts
                                    if(p1+p2<= 1000){//p1 = nombre de pts engrangés avant le lancer de dés
                                        ...
                                        for (h=0; h < edg.size() ; h++){ptchem.push_back(edg[h]+sptopt[h]);}
                                    ...
                                    if (p1+p2> 1000){
                                        ...
                                        nd = (d-sdesopt[i]);
                                        if(nd==0){nd = nd+7;}
                                        if (maxopt > p2){p1 = p1+sptopt[i];p2 = 0;d = nd;}
                                ...
                                else if ((scoreordi>=1000)&&(scoremeilord<=10000)&&(scoreordi<=10000)){//Cas classique
                                    ...
                                    if (scoreordi+p1+p2 >= scoremeilord){//Cas victorieux
                                        ...
                                        for (e=0; e < sdesopt.size(); e++){
                                            nd = (d-sdesopt[e]);
                                            if(nd==0){nd = nd+7;}
                                            if(p1+sptopt[e]>=deltaptt){edg.push_back(ptsmoy[nd]-pperte[nd]*(p1+sptopt[e]));}
                                            else if(p1+sptopt[e]<=deltaptt){
                                                //AVANT:edg.push_back(ptsmoy[nd]-pperte[nd]*(p1+sptopt[e])*exp(beta1*(p1+sptopt[e]-deltaptt)));
                                                //Permet de se fixer un but de points à atteindre, et de prendre un peu plus de risques si on est en-dessous de ce but.
                                                edg.push_back(ptsmoy[nd]-pperte[nd]*(p1+sptopt[e])*(exp(beta1*(p1+sptopt[e]-deltaptt))+gamma1*(p1+sptopt[e]-deltaptt)));
                                                //Changer en fonction linéaire?? En tout cas tester
                                            }
                                        }
                                        for (h=0; h < edg.size() ; h++){
                                            ptchem.push_back(edg[h]+sptopt[h]);
    ...
    //checkcount=0;
    cout << endl << endl << "fvec4: " << fvec4.size() << endl << "fvec5: " << fvec5.size() << endl << endl;
    ...
    if(fvec4.size()>2){
        ...
        if(fvec1.size()!=0||fvec2.size()!=0){
            cout << "Erreur!!" << endl;
            cout << fvec1.size() << endl << fvec2.size() << endl;
            goto END;
        }
        else{
            cout << endl << endl << "tours moyens total: " << double(tourtot)/(counttour*(64+32+16+8+4+2+1));
            cout << endl << "1er ordi" << endl << "beta1: " << fvec4[0] << endl << "gamma1: " << fvec5[0] << endl << endl;
            cout << endl << "2ème ordi" << endl << "beta1: " << fvec4[1] << endl << "gamma2: " << fvec5[1] << endl << endl;
        }
    END: a=1;
    }
}//fin du main
