function [Tak, change] = Tak6 (Tak, dim, change)
changel=0;
for m=1:2
    [Row, Col, nbr, anbr] = TakRowCol (Tak, dim, m);
    for i=1:dim % Row
        if Row.Nbr(i) == (dim/2)-4 && Row.aNbr(i) <= (dim/2)-4
            q=dim-Row.Nbr(i)-Row.aNbr(i);
            o=zeros(q,1);
            Taktmp=Tak; l1=0; l2=0; l3=0; l4=0; p=0;
            for l1=1:q
                check=0; acheck=0;
                for l2=1+p:q
                    if l2 == l1
                    ...
                    for l3=1+p:q
                        ...
                        for l4=1+p:q
                            ...
                            for j=1:dim
                                if Tak(i,j) == 0
                                    k=k+1;
                                    if k == l1-p || k == l2-p || k == l3-p || k == l4-p
                                        Taktmp(i,j)=nbr;
                                    else
                                        Taktmp(i,j)=anbr;
                                    end
    ...
    for j=1:dim % Column
        if Col.Nbr(j) == (dim/2)-4 && Col.aNbr(j) <= (dim/2)-4
